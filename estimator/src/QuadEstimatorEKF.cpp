...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
	...
	// Rotation matrix for converting local frame (p, q, and r) into Euler angle derivatives (phi_dot, theta_dot, and psi_dot)
	...
	// Approximate the integral by multiplying `dt` by the Euler derivatives
	...
	else if (ekfState(6) < -F_PI) ekfState(6) += 2 * F_PI;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
	...
	// Calculate x, y, and z, respectively
	...
	// Calculate x_dot, y_dot, and z_dot, respecitively
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
	...
	// Use equations from "Estimation for Quadrotors" paper - roll=phi, pitch=theta, psi=yaw
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
	...
	// Use equations from "Estimation for Quadrotors" paper
	gPrime(0, 3) = dt;
	gPrime(1, 4) = dt;
	gPrime(2, 5) = dt;
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
	...
	hPrime(0, 6) = 1; 
	zFromX(0) = ekfState(6);
	float yaw_err = magYaw - zFromX(0);
	if (yaw_err > F_PI) zFromX(0) += 2 * F_PI;
	else if (yaw_err < -F_PI) zFromX(0) -= 2 * F_PI;
